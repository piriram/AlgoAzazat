...
for _ in range(N):
    ...
    # 각 카드 묶음의 크기를 입력 받아서 우선순위 큐에 넣는다.

result = 0
while len(queue) != 1:  # 우선순위 큐에 한 개의 원소가 남을 때까지 반복
    num1 = heapq.heappop(queue)
    num2 = heapq.heappop(queue)
    sum_val = num1 + num2 # 우선순위 큐에서 가장 작은 두개의 원소를 꺼낸 두 원소를 합칩니다.

    result += sum_val # 비교 횟수를 누적합해서 다시 큐에 저장
